...
export class RedStoneToken_old implements ERC20 {
  ...
  private readonly _allowances: Map<string, Map<string, u64>> = new Map<
    string,
    Map<string, u64>
  >();
  ...
  allowance(owner: string, spender: string): u64 {
    ...
    let result: u64 = 0;
    if (
      this._allowances.has(owner) &&
      this._allowances.get(owner).has(spender)
    ) {
  ...
  transferFrom(sender: string, recipient: string, amount: u64): void {
    const msgSender = msg.sender();
    console.log(
      `transferFrom called ${sender}[${msgSender}] -> ${recipient}:${amount}`
    );

    if (
      !this._allowances.has(sender) ||
      !this._allowances.get(sender).has(msgSender)
    ) {
  ...
  private _transfer(sender: string, recipient: string, amount: u64): void {
    ...
    if (senderBalance < amount) {
      throw new ContractError(
        `Caller balance not high enough to send ${amount} token(s)!`
      );
    }
